...
def count_randomized_sequences(biosamples, filtered_file_list, working_folder_name,
    ...
    print(list(np.arange(len(biosamples))))
    for i in list(np.arange(len(biosamples))):
        ...
        print('/nProcessing',i, biosamples[i])
        ...
        miss_count=0
    for sequence in possible_sequence_dict:
        if possible_sequence_dict[sequence][0]==0:
            miss_count+=1
            print(sequence)
            #print('missed sequence:',sequence)
            continue

    print('miss sequences number:',miss_count)
