...
from VAMP_processing import VAMP_processing


# SETUP

# Copy the parameter file name and enter it here as a raw string (r'string')
parameter_file_name = r"D:\Boston_college\PhD_PROJECT\Writing\VAMP\Fastq files\MmPyl-VAMP\Main_MmPyl_VAMP.csv"
...
VAMP_para = r"D:\Boston_college\PhD_PROJECT\Writing\VAMP\Fastq files\MmPyl-VAMP\Analysis_MmPyl_VAMP.csv"
...
#EXTRA POSITION BASE FREQUENCY CALCULATION
if run_VAMP_analysis:
    with open(rf'{p["twist_sequence_file"]}', 'r', encoding='utf-8', newline='') as sequence_file:
        sequence_file_reader = csv.reader(sequence_file)

        # quick change from T to U.
        possible_VAMPsequence_dict = {line[0].upper().replace('T', 'U'): 0 for line in sequence_file_reader}
    VAMP_processing(p["working_folder_name"], p["lib_name"], biosamples, p["randomized_bases"][0],p["randomized_bases"][-1], str(dt.date.today()),
                    possible_VAMPsequence_dict, VAMP_para)
